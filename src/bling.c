 ...
 * @bling: the PhocBling that needs to damage output(s)
 *
 * A utility method to damage the entire bounding box region of a PhocBling.
 */
void
phoc_bling_damage_box (PhocBling *bling)
{
  PhocDesktop *desktop = phoc_server_get_desktop (phoc_server_get_default ());
  PhocOutput *output;
  struct wlr_box damage_box;

  if (!phoc_bling_is_mapped (bling))
    return;

  damage_box = phoc_bling_get_box (bling);
